...
const TaskItem: React.FC<Props> = ({ task, idx, groupIdx }) => {
  ...
  return (
    <div
      className="bg-lime-200 p-4 text-gray-600 rounded mb-4 w-[186px] h-[160px]"
      draggable={draggable}
      ref={item}
      style={{
        left: position.x,
      ...
      onDrag={(e) => {
        ...
        if (newParentIdx == currentGroupIdx) {
          ...
          // 位置和组别都发生变化
          let newParent = document.getElementById(
            `task-container-${newParentIdx}`
          )
